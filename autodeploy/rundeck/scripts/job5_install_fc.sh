...
echo "[job5] starting — Install Failover Clustering ($(date -Is))"
echo "[job5] user=$(whoami) host=$(hostname -f 2>/dev/null || hostname)"
...
command -v az >/dev/null || { echo "[job5] ERROR: az not found"; exit 1; }
