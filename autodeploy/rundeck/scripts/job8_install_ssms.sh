...
echo "[job8] starting — Install SSMS ($(date -Is))"
echo "[job8] user=$(whoami) host=$(hostname -f 2>/dev/null || hostname)"
...
echo "[job8] target VM: $VM_NAME"
echo "[job8] ansible_workdir=$ANSIBLE_WORKDIR"
...
command -v az >/dev/null || { echo "[job8] ERROR: az not found"; exit 1; }
command -v ansible-playbook >/dev/null || { echo "[job8] ERROR: ansible-playbook not found"; exit 1; }
...
echo "[job8] reading localadmin password from Key Vault: $KEYVAULT_NAME/$LOCALADMIN_SECRET_NAME"
...
ansible-playbook -i "$INVENTORY_RUNTIME" playbooks/install-ssms.yml -v

echo "[job8] done ($(date -Is))"
