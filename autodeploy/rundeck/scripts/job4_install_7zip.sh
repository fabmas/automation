...
echo "[job4] starting — Install 7-Zip ($(date -Is))"
echo "[job4] user=$(whoami) host=$(hostname -f 2>/dev/null || hostname)"
...
echo "[job4] ansible_workdir=$ANSIBLE_WORKDIR"
echo "[job4] inventory=$ANSIBLE_INVENTORY"

az login --identity --output none || true
az account set --subscription "$AZ_SUBSCRIPTION_ID" >/dev/null

command -v az >/dev/null || { echo "[job4] ERROR: az not found"; exit 1; }
command -v ansible-playbook >/dev/null || { echo "[job4] ERROR: ansible-playbook not found"; exit 1; }

cd "$ANSIBLE_WORKDIR"
test -f "$ANSIBLE_INVENTORY" || { echo "[job4] ERROR: inventory not found: $ANSIBLE_INVENTORY"; exit 1; }
...
echo "[job4] reading localadmin password from Key Vault: $KEYVAULT_NAME/$LOCALADMIN_SECRET_NAME"
...
echo "[job4] runtime inventory created"
unset LOCALADMIN_PASSWORD

ansible-playbook -i "$INVENTORY_RUNTIME" playbooks/install-7zip.yml -v

echo "[job4] done ($(date -Is))"
